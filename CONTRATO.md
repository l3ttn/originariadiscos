...
- **Ficha** (`disco.html`): botão primário "Adicionar ao carrinho" acima dos dois CTAs atuais (Avise-me / Encontre pra mim), que continuam.
...
```
  ...
  `(ano)` só se houver; `· N un.` só se `qtd > 1`; a linha do Discogs usa `discogsUrl` sem `https://www.`; preço formatado `R$ 220` sem centavos; itens na ordem em que foram adicionados.
